...
export default function Filters({ onApply }) {
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/products/highest_price`).then((res) => {
      const price = Number(res.data.data?.highest_price || 0);
      setMaxPrice(price);
  ...
  useEffect(() => {
    axios.get(`${API_URL}/api/categories/categories_get_all_api`).then((res) => {
  ...
  const applyFilters = () => {
    ...
    const filters = {
      ...
      categoryIds: selected.length > 0 ? selected : null,
    };

    onApply(filters); // gửi filter ra cha
  };
